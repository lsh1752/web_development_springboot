...
/*
    *의 의미는 SQL문에서도 배우겠지만
    all의 뜻을 지니고 있습니다.
    즉, org.junit.jupiter.api 이하의 모든것들을 import 하겠다는 의미
 */
...
/*
    @BeforeAll
        ...
        실행 주기에서 한 번만 호출돼야 하기 때문에 메서드를 static으로선언(정적 메서드)

    @BeforeEach
        테스트 케이스를 시작하기 전에 매번 실행
        테스트 메서드에서 사용하는 개체를 초기화하거나 테스트에 필요한 값을 미리
            넣을 때 사용할 수 있음
    ...
    @AfterAll
        ...
        실행 주ㅜ게이서 한 번만 호출돼야 하기 때문에 메서드를 static으로 선언(정적 메서드)
