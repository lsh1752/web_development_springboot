...
-- 표시 컬럼 country, uniqueUserCnt(Group by - 0)
...
-- select - from 테이블명 - group by절 - order by 절 : 순서
...
예를 등어서 위의 쿼리문의 경우, country를 기준으로 먼저 그룹화가 이루어지고,
...
-- GROUP BY 정리
-- 1) 주어진 컬럼을 기준으롤 데이터를 그룹화하여 '집계함수'와 함께 사용
-- 2) GROUP BY 뒤에는 그룹화할 컬ㄹ럼명을 입력. 그룹화한 컬럼에 집계 함수 적용하여
		-- 그룹별 계산을 수행.
-- 3) 형식 : GROUP BY 컬럼명1, 컬럼명2, ...
-- 4) GROUP BY에서 두 개 이상의 기준 컬럼을 조건으로 추가하여 여러 그룹으로
-- 		분할 가능한데, 컬럼 순서에 따라 결과에 영항을 미치므로 우선순위를 생각할 필요 있음
...
SELECT SUBSTR(created_at, 1, 7) AS 월, COUNT(DISTINCT id) AS uniqueUserCnt 
	FROM users
	GROUP BY SUBSTR(created_at, 1, 7) 	-- 이거 확인 후에 말씀드리겠습니다.(month)사용
...
-- users에서 회원 수가 8명 이상인 country 별 회원 수 출력(회운 수 기준 내림 차순)
...
-- 주문 회원수가 40명 이하인 데이터만 출력(단, 주문 건수 기준으로 내림차순 정렬할 것.
...
-- HAVING 정리
-- 순서상 GROUP VY 뒤쪽에 위치하며, GROUP BY와 집계함수로 그룹별로 집계한 값을
-- 필터링할 때 사용
