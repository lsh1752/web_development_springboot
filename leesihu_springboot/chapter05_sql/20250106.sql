...
-- 이상의 SQL문에 대한 해서
-- 기존에 from 다음에는 테이블 명 하나만 작성되ㅣ었지만, 이제는 JOIN연산을 위한
-- 추가 문법이 적용됐음.
...
-- 여러 테이블을 한나의 FROM에서 다룰 때에는 별칭을 사용 가능(ALIAS와는 다릅니다 -> 컬럼명을 지정).
...
-- 이사으이 문제에서의 조건은 '주문 정보가 있는 회원의 정보만 출력'하는 것이므로, orders 내에
-- user_id가 일부 기준이 되어야 합니다.

-- 왜냐하면 users 내에 있는 id는 1부터 끝까지 있으니까요.
-- users 테이블에는 회원id가 id 컬럼에 기록돼있고, orders 테이블에는 회원 id가 user_id로
-- 공통된 부분을 지정하는 컬럼이 존재하므로 둘을 연결시킬 수 있는데, 이때 사용하는 전치가사
-- 'ON'

-- JOIN 적용 후 결과를 보기 좋게 정렬하도록 SQL 문 수정
-- 회원 id를 기준으로 오름차순하는 조건.
-- ORDER BY에서도 테이블 별창으로 정렬할 컬럼을 지정 가능.

SELECT *
	FROM users u INNER JOIN orders o ON u.id = o.user_id
	ORDER BY u.id;

-- FROM에서 FOIN이 정렬된 후데 단일 테이블에 명령을 내리는 것처럼 쿼리를 작성 가능
-- -> 이미 JOIN을 통해 하나의 테이블로 구성된 것처럼 간주되기 때문.

-- 복수의 테이블이 하나로 결합되기 위해서는 두 테이블 간에 공통된 부분이 존재해야 합니다.
-- RDBS에서는 이 부	분을 키(key)라고 합니다.
-- 키 값은 테이블에 반드시 1 개 이상 존재하도록 설계되어 있고(여러분이 설계 해야 하고),
-- 데이블에서 개별 행을 유일하게 구분 짓습니다. 따라서 키 값은 컬럼 내에서 중복되지 않으며
...
-- users의 u.id가 PK에 해당해서 FK가 PK를 참조해서 조건을 합치시커 JOIN함.)
...
-- 왼쪽 테이블의 값을 전부ㅜ 불러오기 때문에 LEFT JOIN / LEFT OUTER JOIN이라고 합니다.

-- LEFT JOIN과 INNER JOIN은 합께 실무에서 자주 쓰입니다. 데이터를 결합하는 경우 대부분
-- 한쪽 데이터의 값을 보존해야 할 때가 많은데, 이번 예시에서도 그렇습니다. 주문정보가 없는
...
-- u.id, u.username, o.orderdate처럼 컬럼이 속한 테이블 별칭을 . 앞에 명시해주면 됩니다.
...
-- 테이블의 모든 값을 가져왔습니다(즉, JOIN했을 때를 ㅣㄱ준으로 u.id는 있지만 o.user_id가 없는 경우도)
-- (출력된다는 것을 의미합니다.)
...
-- 따라서 orders 테이블의 user_id 컬럼은 보두 users 테이블의 id 컬럼에 있는 값에 해당합니다.
-- 결합으로 봤을 때 order는 users 테이블에 종속돼있으므로 null 값이 출력될 수 없습니다.

-- 이상을 이유로,
-- 많은 기업에서는 FIGHT JOIN 대신에 LEFT JOIN을 사용하도록 권장합니다.

-- users와 orders의 모든 가능항 행 조합을 만들어 내는 쿠ㅝ리를 작성합니다.
SELECT *
...
-- u.id를 기준으로 정렬했습니다(얼마나 많은 조합이 나오는지 시각적으로 보여드를 수 있도록 추가함)
...
SELECT u.country, count(DISTINCT o.id) AS count
	FROM users u LEFT JOIN orders o ON u.id = o.user_id
...
-- 일단 JOIN이 수행된 두에 다른 명령어들이 수행됩니다. JOIN 사용 시, 결합할 두 테이블 사이에
...
-- 정리 2. JOIN 중첩
-- FROM [테이블명1] a(별칭) [INNER/LEFT/FIGHT/CROSS JOIN] [테이블명2] b(별칭)
-- ON [JOIN 조건1]
-- [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명3] c(별칭)
-- ON [JOIN 조건2]

-- FROM 내에서 JOIN을 여러 번 중첩 사용 가능, 앞의 JOIN의 ON 절 뒤에 새로운 JOIN 명령어를
-- 작성하면 제 3의 테이블과결합 가능 -> 순서대로 적용한다는 점이 중요합니다.
-- 회수 제한은 없지만, 테이블 크기에 따라 많은 연산이 필요할 수 있어 사전에 필요한 연산인지
-- 점검할 필요가 있습니다.
...
-- INNER JOIN은 각 테이블의 키 값이 '일치하는 힝만' 가져옵니다. 집합으로 보면 교집합.
...
-- FULL OUTER JOIN : 왼쪼고가 오른쪽에 있는 테이블의 모든 값을 결과에 포함
-- 			LEFT + RIGHT JOIN의 결과값이 중복없이 결홥된 형태.
