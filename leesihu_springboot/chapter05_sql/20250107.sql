...
-- 이상의 쿼리의 문제점은 결합 기능을 지닌 UNION의 결과값이 select * form users;와
-- 동일하다는 점입니다.
...
(SELECT id, phone, city, country	-- 기준이 되는 첫번째 select절에서 선택하는
	FROM users)						-- 컬럼의 종유 및 개수가 완벽히 일치해야 합니다.
	ORDER BY id;
...
-- (SELECT *
-- 	FROM orders
-- 	WHERE order_date BETWEEN "2015-10-01" AND "2015-10-31")
-- UNION ALL 
-- (SELECT *
-- 	FROM orders
...
-- 3. 이건 풀기 X 문제만 적고 같이 풀겠습니다.
-- 	UNION을 활용하여 orderdetails와 products를 FULL OUTDER JOIN 조건으로 결합하여 출력하세요
--  궅이 이런 형식으로 시험 문제 및 실무에까지 이용하는 경우는 거의 없습니다
...
-- SELECT AVG(price) FROM products; 가 요구된다는 점에서 효울적이지는 않고,
...
-- 2번 쿼리가 서브 쿼리 포함된 해설 -> 해당 문제에서 눈여겨 볼 것은 궅이 따지자면
-- 여기서는 서브쿼리를 작성하는데 있어서 스칼라 서브 쿼리 형태로 작성하는 것이 중요함
-- 해당 문제에서는 검증 결과 서브 쿼리 자체가 필요하지는 않다.
-- a 이 부분도 비표준 SQL이라서 일단은 적어만 둠
	
-- orders와 staff를 활용해 last_name 이 Kyle이나 Scott인 직원의 담당 주문을 출력하려면?
-- (단, 서브쿼리 형태를 활용하자)
SELECT id
	FROM STAFF S WHERE last_name IN ("Kyle", "Scott");			-- 조건절에 쓰일 경우에
															-- scalar 서브가 아니었다는점에 주목

-- 이상의 코드는 staff 테이블에서 id 값이 3, 5를 도출해냈습니다.
-- 해당 결괄를 가지고 orders 테이블에 적용하는 형태로 작성합니다.
...
-- 이사의 쿼리문의 해석 -> 직원 자신이 자기 쇼핑몰에서 주문한 이력이 반환된 것
...
-- 이후 해당 쿼리가 scalar냐 아니냐에 따라서 그 위치 역시 어느 정도 통게 가능
-- ex) scalar인 경우네는 select절에 들어가는 것처럼
...
-- 여러 거ㅐ의 컬럼을 추가하고 싶을 때는 서브 쿼리를 여러 개 작성하면 됩니다.
-- 특징 : SELECT의 서브 쿼리는 메인 쿼리의 FROM에서 사용된 테이블이 아닌 테이블의 사용이 가능하기 
...
-- FROM에서 사용되는 서브 쿼리 : 인라인 부, 마치 테이블처럼 서브 쿼리의 결과값을 사용 가능.
...
-- 테이블 검색을 빠르게 할 수 있는 인덱스* 개념이 있는데 서브 쿼리를 활용하면 인텍스를 사용하지 못하는
-- 경우가 있으므로 주의해야 함.
