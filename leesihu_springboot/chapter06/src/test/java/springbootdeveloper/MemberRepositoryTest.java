...
import me.leesihu.springbootdeveloper.Member;
import me.leesihu.springbootdeveloper.MemberRepository;
...
class MemberRepositoryTest {
    ...
    void getAllmembers() {
        ...
        // 우리가 직접 정의하는 영어가 이니라도 익숙해져야 합니다.

        // then
        assertThat(members.size()).isEqualTo(3);
    }
    /*
        이상의 코드느 ㄴ멤버 전체를 찾고, 그 리스트의 사이즈가 3인지를 확인하는 테스트

        이하의 코드는 id 2인 멤버를 찾아보도록 합니다.
        ex)
        SELECT * FROM member WHERE id = 2;
        member라는 테이블의 모든 컬럼을 조회하는데, id가 2인 객체읙 값들을 보여달라.
    ...
    @Sql("/insert-members.sql")
    @Test
    void gitMemberByName() {    // MemberRepository.java에는 findByName()이었다는 점에ㅔ 주목
        // when
    ...
    /*
        ...
        일반적인 겨우 JPA가 정해준 메서드 이름 규칙을 따르면 쿼리문을 특별히 구현하지 않아도
        메서드처럼 사용할 수 있음(즉 자바만으로 컨트롤이 가능하다는 의미)

        이사으이 메서드를 SQL로 작성할 겨우
        SELECT * FROM member WHERE name = 'C';

        지금까지 조회 고나련 메서드 수업했습니다.
        전체 조회 -> findAll()
        아이디로 조회 -> findById()
        특정 컬럼으로 조회(name) -> 쿼리 메서드 명명 규칙에 맞게 저으이 후에 사용(find / get 차이)

        추가 / 삭제 메서드

        INSERT INTO member (id, name) VALUES (1, 'A');라는 퀄리가 있다고 가정했을 때
    ...
    void saveMembers() {
        ...
        List<Member> members = List.of(new Member(2L, "이시후"),
                new Member(3L, "안근수"));
    ...
    /*
        ...
        JPA에서드 deleteById()를 통해 레코드를 삭제 가능.
