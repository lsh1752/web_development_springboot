...
public class BlogService {
    ...
    /*
        JPA 지원 메서드인 findAll()을 호출해 article 테이블에 저장돼 있는 모든 데이터를 조회 -> Test 에서 이미 해봤습니다.

        응답을 위한 DTO 생성 -> dto package 에 -> ArticleResponse.java 파일 생성
     */

    // id로 특정 글 조회
    public Article findById(long id) { // 이 경우 결과값은 하나밖에 없기 때문에 리턴 타입이 Article 이어야만 합니다. -> id PK 라서
