...
import me.leesihu.springbootdeveloper.dto.ArticleResponse;
...
public class BlogApiController {
    ...
    /*
        ...
        클ㄹ라이언트에게 전송합니다 -> 해당 코드에서는 stream을 적용했습니다 -> 추후 설명

        * stream : 여러 데이터가 모여 이쓴ㄴ 컬렉션을 간편하게 처리하기 위해서 사용하는 기능
            자바 8에 추가
     */
    @GetMapping("/api/articles/{id}")
    // URL 경로에서 값을 추출
    public ResponseEntity<ArticleResponse> findArticle(@PathVariable long id) {// URL에서 {id}에 해당하는 값이 id 로 들어옴
    ...
    /*
        ...
        조회 성공하셨으면
        DELETE 로 HTTP 메서드를 바꿔주고,
        http://localhost:8080/api/articles/1 하고 Send 버튼 누릅니다.

        GET 으로 HTTP 메서드 바꿔주고,
        http://localhost:8080/api/articles/1 -> 이건 지원졌기 때문에 조회 x


    http://localhost:8080/api/articles -> 얘로 조회해서 전체
     */
