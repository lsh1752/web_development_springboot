...
/*
    1. 스프링 부트 3 구조 살펴보기
        ...
        계픙 : 각자의 역할과 책임이 있는 어떤 소프트웨어의 구성 요소
            -> 각 계층은 소통할 수는 있지만 다른 계층에 직접 간섭하거나 영향을 미치진 않음

        ex) 카페와 빵집
            카페이서는 커피를 팔고, 빵집에서는 빵을 파는데, 필요한 경우 서로 협업 관계를 맺어
        ...
        스프링 부트에서의 계층
            ...
            3) 퍼시스턴스(Persistence)
                모든 데이터베이스 관련 로직을 처리. 이 과정에서 데이터베이스에 접근하는
                    DAO(추후 수업 예정0 객체를 사용할 '수도' 있음.
                        * DAO - 데이터베이스 계층과 상호작용하기 위한 객체라고 이해하면 편합니다 일단은.
                        -> Repository가 퍼시스턴스 계층의 역할을 함.

            계층은 개념의 역역이고 Controller, Service, Repository는 실제 구현을 위한 영역
                -> 추후 수업(아마 목요일) 예정입니다.

        스프링 부트 프로젝트 디렉토리 구성
            : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발자들이 따르드
                수업 시에도 해당 구조를 따라 개발할 예정.

                MAIN : 실제 코드를 작성하는 공간, 프로젝트 실행에 필요한 소스 코드나 리소스 파일은
                    모두 이 폴더 안에 들어있음(프로젝트 새엇ㅇ시 자동 구현됨)

                test : 프로젝트의 소스 코드를 테스트할 목적의 코드나 리소스 파일이 3들어잇음
                    (원래는 자동생성되는데 어제 제가 만들었을 때 안만들어지는 걸로 보아 살정 딸라
                    다른듯합니다)

                build.gradle : 빌드를 설정하는 파일. 의존성이나 플러그인 설정광 같이 빌드에 필요한
                    설정을 할 때 사용
                ...
                main 디렉토리 구성하기
                    ...
                    01 단계 - HTML과 같은 뷰 관련 파일(chapter07에서 수업 예정)
                        resources 우클릭 -> new directory -> templates
                    02 단계 - static 디렉토리 JS, CSS, 이미지와 같은 정적 파일을 넣는 요도로 사용
                        -> 이미 만들었으므로 생략합니다.
    ...
    2. 스프링 부트 3 프로젝트 발전시키기
        ...
        각 계층의 코드를 추가할 예정 -> 이를 통해 계층이 무엇이고, 스프링 부트에서는 계층을 어덯게
        ...
        annotationProcessor : 컴파일 시에 에너테이션을 처리할 때 사용하는 도구의 의존성 지정
    ...
    지시 사항
        ...
        02 단계 - 이제는 기존에 만들어둔 application.yml 파일

        03 단계 - 서버 실행 후에 ctrl + f 눌러서 create ㄱ머색해서 table이 생성됐는지 확인
