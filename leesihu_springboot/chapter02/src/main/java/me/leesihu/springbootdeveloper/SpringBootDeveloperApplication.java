...
/*
    New project 생성시 주의점 :
        ...
        2. DLS -> Groovy 설정
        3. name = ArtifactId
        4. build.gradle 설정을 복사하는데, 복사 후에 -> sync를 꼭 눌러줘야 함.
            -> SpringBootDeveloperApplication에서 @SpringBootApplication 애너테이션에 빨간줄 가있으면
                새로고침 안했을 확률이 거의 90%
        5. resources 내에 static 내에 index.html 이라고 하는데, 해당 폴더명의 경우 대부분의 개발자들이 합의한 형태
        5. 앞으로의 수업 시간에는 new project 생성시에 이렇게 자세히 풀이할 일은 자주 있지 않을 예적이니 꼭
...
/*
    1. Spring vs SpringBoot
        ...
        SpringBoot의 등장
            ...
            특징 :
                1) 톰캣, 제티, 언더토우 같은 웹 애플리케이션 서버가 내장돼있어따로 설치하지
                    않아도 독집적으로 실행 가능
            ...
            차이점 :
                1) 구성의 차이 -> 스프링은 애플리케이션 개발에 필요한 환경을 수동으로 구성하고 정의
                    하지만 스프링부트는 스프링 코어와 스프링 MVC의 모든 기능을 자동으로 고드
            ...
            4) 관점 지향 프로그래밍(AOP)
                Aspect-oriented Programming : 프로그래밍에 대한 관심을 핵심 관점 / 부가 관점
                    ...
                    ex) 계좌 이체, 고객 관리하는 프로그램이 있다고 가정할 때, 각 프로그램에는
                        ...
                        하기 위한 데이터베이스 연결 로직이 포함된. 이 때,
                            핵식 관점 - 계좌 이체, 고객 관리 로직
                            부가 관점 - 로깅, 데이터 베이스 연결 로직

            5) 이식 하능한 서비스 추상화(PSA)
...
3. 스프링 부트 3 둘러보기
    ...
    스프링 부트 스타터 살펴보기
        ...
        spring-boot-starter-text : 스프링 애플리케이션을 테스트하기위해 필요한 의존성 모음
